%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% misfit for rms amplitude and cc time shift
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
cc=xcorr(data,synth_tap_weighted);
tcc=-(length(t)-1)*dt:dt:(length(t)-1)*dt;

%--------------------------------------------------------------------------
%- find maximum 
%--------------------------------------------------------------------------

cc=cc.*exp(-tcc.^2/60^2);

shift_index=min(find(cc==max(cc)));
shift_time=-tcc(shift_index);
Ep=shift_time;

if (abs(Ep)>misfit_upper_bound)
    Ep=0;
...
ad_src=shift_time*synth_tap/(dt*sum(synth_tap.*synth_tap));
iv=length(t):-1:1;
