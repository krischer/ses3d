...
implicit none
	...
	real, allocatable, dimension(:,:,:) :: p_csv, p_csh, p_cp, p_rho
	real, allocatable, dimension(:,:,:) :: p_lambda, p_mu, p_b, p_rhoinv
	real, allocatable, dimension(:,:,:) :: p_normalisation

	real, dimension(0:nx_max,0:ny_max,0:nz_max,0:lpd,0:lpd,0:lpd) :: normalisation
	...
	write(*,*) 'directory for model files: '
	read(*,*) fn_model
	write(*,*) 'confirm directory: ', fn_model
	...
	open(unit=120,file=fn_output(1:len_trim(fn_output))//'vsh',action='write')
	open(unit=130,file=fn_output(1:len_trim(fn_output))//'vsv',action='write')
	open(unit=140,file=fn_output(1:len_trim(fn_output))//'vp',action='write')
	open(unit=150,file=fn_output(1:len_trim(fn_output))//'rho',action='write')
	...
	do isubvol=1,nsubvol
		...
		allocate(p_csh(1:nbx-1,1:nby-1,1:nbz-1))
		allocate(p_csv(1:nbx-1,1:nby-1,1:nbz-1))
		allocate(p_cp(1:nbx-1,1:nby-1,1:nbz-1))
		allocate(p_rho(1:nbx-1,1:nby-1,1:nbz-1))

		allocate(p_mu(1:nbx-1,1:nby-1,1:nbz-1))
		allocate(p_lambda(1:nbx-1,1:nby-1,1:nbz-1))
		allocate(p_b(1:nbx-1,1:nby-1,1:nbz-1))
		allocate(p_rhoinv(1:nbx-1,1:nby-1,1:nbz-1))

		allocate(p_normalisation(1:nbx-1,1:nby-1,1:nbz-1))
		...
		do ind=1,p
			...
			z(k,n)=zmax-k*dz-0.5*(1+knots(n))*dz
		      ...
		      fn=fn_model(1:len_trim(fn_model))//'rhoinv'//junk(1:len_trim(junk))
		      ...
		      fn=fn_model(1:len_trim(fn_model))//'lambda'//junk(1:len_trim(junk))
		      ...
		      fn=fn_model(1:len_trim(fn_model))//'mu'//junk(1:len_trim(junk))
		      ...
		      fn=fn_model(1:len_trim(fn_model))//'B'//junk(1:len_trim(junk))
		      ...
		      do indz=1,nbz-1
				...
				do n=0,lpd
				  ...
				  p_normalisation(indx,indy,indz)=p_normalisation(indx,indy,indz)+normalisation(i,j,k,l,m,n)*intx*inty*intz/(dbx*dby*dbz)

				  p_rhoinv(indx,indy,indz)=p_rhoinv(indx,indy,indz)+rhoinv(i,j,k,l,m,n)*intx*inty*intz/(dbx*dby*dbz)
				  p_lambda(indx,indy,indz)=p_lambda(indx,indy,indz)+lambda(i,j,k,l,m,n)*intx*inty*intz/(dbx*dby*dbz)
				  p_mu(indx,indy,indz)=p_mu(indx,indy,indz)+mu(i,j,k,l,m,n)*intx*inty*intz/(dbx*dby*dbz)
				  p_b(indx,indy,indz)=p_b(indx,indy,indz)+B(i,j,k,l,m,n)*intx*inty*intz/(dbx*dby*dbz)
		...
		p_mu=p_mu/p_normalisation
		p_lambda=p_lambda/p_normalisation
		p_b=p_b/p_normalisation
		p_rhoinv=p_rhoinv/p_normalisation

		p_csv=sqrt((p_mu+p_b)*p_rhoinv)/1000
		p_csh=sqrt(p_mu*p_rhoinv)/1000
		p_cp=sqrt((p_lambda+2*p_mu)*p_rhoinv)/1000
		p_rho=1/(1000*p_rhoinv)

		do i=1,nbx-1
		do j=1,nby-1
		do k=1,nbz-1

		  write(120,*) p_csh(i,j,k)
		  write(130,*) p_csv(i,j,k)
		  write(140,*) p_cp(i,j,k)
		  write(150,*) p_rho(i,j,k)
