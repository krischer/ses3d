subroutine ses3d_make_slice_model
use parameters
use variables
implicit none
		...
		if ((plane==1) .and. (val<=xmax) .and. (val>xmin)) then
			...
			do comp=1,5
				...
				if (opened==1) then
					...
					elseif (comp==2) then
						fn=dir(1:len_trim(dir))//'slice_x_mu'//junk(1:len_trim(junk))
					elseif (comp==3) then
						fn=dir(1:len_trim(dir))//'slice_x_lambda'//junk(1:len_trim(junk))
					elseif ((comp==4) .and. (is_diss==1)) then
						fn=dir(1:len_trim(dir))//'slice_x_logtau'//junk(1:len_trim(junk))
					elseif (comp==5) then
		...
		elseif ((plane==3) .and. (val<=zmax) .and. (val>zmin)) then
			...
			do comp=1,5
				...
				if (opened==1) then
					...
					elseif (comp==2) then
						fn=dir(1:len_trim(dir))//'slice_z_mu'//junk(1:len_trim(junk))
					elseif (comp==3) then
						fn=dir(1:len_trim(dir))//'slice_z_lambda'//junk(1:len_trim(junk))
					elseif ((comp==4) .and. (is_diss==1)) then
						fn=dir(1:len_trim(dir))//'slice_z_logtau'//junk(1:len_trim(junk))
					elseif (comp==5) then
