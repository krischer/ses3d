	  ...
	  do k=start_z,end_z
	    ...
	    where ((cox>=bx(i)) .and. (cox<bx(i+1)) .and. &
		   ...
		   (coz>bz(k)) .and. (coz<=bz(k+1)))
		      ...
		      d_mu=d_beta_sh(i,j,k)*d_beta_sh(i,j,k)*d_rho(i,j,k)
		      d_b=d_beta_sv(i,j,k)*d_beta_sv(i,j,k)*d_rho(i,j,k)-d_mu
		      d_lambda=d_alpha(i,j,k)*d_alpha(i,j,k)*d_rho(i,j,k)-2*d_mu
